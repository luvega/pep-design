#!/usr/bin/env python3
"""Merge v0.31 bounded Wave A pilot runtime outputs into compact KB tables."""
...
def wave_a_jobs(job_manifest: Path) -> list[dict[str, str]]:
    return [row for row in read_csv(job_manifest) if row.get("execution_wave") == "wave_a"]


def execution_matrix_rows(execution_matrix: Path) -> dict[str, dict[str, str]]:
    if not execution_matrix.is_file():
        return {}
    return {row.get("job_id", ""): row for row in read_csv(execution_matrix)}
...
def failed_candidate_row(job: dict[str, str], reason: str) -> dict[str, str]:
    return {
        ...
        "notes": "Bounded Wave A pilot missing or failed parser output; not Benchmark result; not scoring evidence",
    }
...
def run_row_from_candidate(candidate: dict[str, str], job: dict[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        "notes": "Bounded Wave A pilot row only; not Benchmark result; not scoring evidence",
...
def execution_row_from_outputs(
    *,
    ...
    return {
        "execution_id": execution.get("execution_id", f"exec_{job['job_id']}_v031"),
        "job_id": job["job_id"],
        "method": job.get("method", ""),
        "target_id": job.get("target_id", ""),
        ...
        "next_action": (
            "Keep as bounded pilot parser evidence; do not score or rank."
            if status == "parsed"
            else "Inspect method logs and adapter contract before scoring or rerun."
        ),
    }


def merge_v031_outputs(
    *,
    ...
    for job in jobs:
        ...
        job_method_rows: list[dict[str, str]]
        job_candidate_rows: list[dict[str, str]]
        if method_path is not None and method_path.is_file():
            job_method_rows = read_csv(method_path)
        else:
            job_method_rows = [failed_method_row(job, job_dir, "missing_method_output_manifest")]
        method_rows.extend(job_method_rows)

        if candidate_path is not None and candidate_path.is_file():
            job_candidate_rows = read_csv(candidate_path)
        else:
            job_candidate_rows = [failed_candidate_row(job, "missing_candidate_outputs")]
    ...
    write_csv(out_dir / "pilot_method_output_manifest_v0.31.csv", METHOD_OUTPUT_HEADERS, method_rows)
    write_csv(out_dir / "pilot_candidate_outputs_v0.31.csv", CANDIDATE_HEADERS, candidate_rows)
    write_csv(out_dir / "pilot_run_v0.31.csv", RUN_HEADERS, run_rows)
    write_csv(
        deployment_dir / "pilot_execution_results_v0.31.csv",
        EXECUTION_RESULT_HEADERS,
        execution_rows,
    )
    (out_dir / "pilot_v031_merge_summary.json").write_text(
        json.dumps(
            {
                ...
                "evidence_boundary": "Bounded Wave A pilot merge only; not Benchmark result; not scoring evidence",
            },
            indent=2,
            sort_keys=True,
        )
