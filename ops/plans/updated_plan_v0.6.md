...
- 新增 `kb/tables/ars_review_action_items_v0.6.csv`，把 ARS 评审意见变成可裁决 action items。
- 新增 `benchmark/input_sets/dataset_supplement_watchlist_v0.6.csv`，记录 PepBenchmark、GPCR peptide benchmark、TCRTransBench 等新增或强化来源，但不进入 frozen target set。
- 新增 `benchmark/deployment/server_smoke_test_contract_v0.6.md`，规定后续服务器端 clone/download/install/smoke test 的 gating 顺序。
...
5. Refresh `manuscript/outlines/benchmark_manuscript_outline.md` Results and Discussion sections to include v0.5/v0.6 readiness.
