...
候选数据集记录在 `benchmark/input_sets/candidate_benchmark_datasets.csv`。`target_set_v0.csv` 仍保持 schema-only，后续只有在完成来源、许可、assay、positive/negative controls 和 leakage/homology 审计后，才会写入真实 target rows。
