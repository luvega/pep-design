...
v0.3 新增 `benchmark/environments/`，用于评估 10 个 include 方法的环境可行性、GPU/CPU 需求、关键依赖、许可阻塞和 smoke-test 优先级。当前阶段不创建 conda 环境、不安装软件、不下载模型权重，也不运行任何方法。

主表为 `benchmark/environments/environment_feasibility_matrix.csv`。
