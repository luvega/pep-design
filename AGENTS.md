...
Current authoritative plan: `reports/updated_plan_v0.9.md`.
...
- Treat `references/`, `tables/`, `wiki/`, `reports/`, and `benchmarks/` as generated project artifacts.
- Treat `benchmarks/` as the Benchmark protocol, readiness-audit, smoke-test planning, and future run-result interface layer.
...
|:---|:---|
| Maintain KB structure, indexes, logs, raw/wiki/schema boundaries, or `AGENTS.md` | `building-llm-wiki` |
...
|:---|:---|:---|
| `reports/updated_plan_v0.9.md` | current plan | planning artifact only |
| `reports/updated_plan_v0.6.md` | historical ARS baseline | not the current plan |
| `reports/license_schema_input_contract_review_v0.8.md` | readiness evidence | not download/install/run evidence |
| `benchmarks/deployment/download_manifest_v0.8.csv` | future server download route | all rows must keep `download_performed=no` until approved server execution |
| `benchmarks/deployment/method_readiness_review_v0.8.csv` | method license/env/checkpoint/input-contract audit | not local reproducibility evidence |
| `benchmarks/method_sources/method_landscape_watchlist_v0.9.csv` | method landscape and Related Work coverage | not an include scorecard or source pin audit |
| `benchmarks/input_sets/target_set_v0.csv` | frozen target interface | remains schema-only until controls, assay, license, leakage, and provenance are complete |
| `benchmarks/input_sets/example_run.csv` | artificial input-contract example | rows must remain `status=not_real_benchmark` |
| `reports/benchmark_manuscript_claim_evidence_map.csv` | claim gate | unsupported claims must stay unsupported |
| `reports/wiki_validation_report.md` | latest machine validation result | regenerate through validator, do not hand-edit counts |
...
- Do not treat source pinning, method contracts, availability checks, download manifests, or schema reviews as execution evidence.
...
1. Inspect `index.md`, `reports/updated_plan_v0.9.md`, and `reports/wiki_validation_report.md`.
2. Refresh source metadata only when the user requests it or the task depends on current external state.
3. Update `references/` and `tables/` before regenerating wiki or reports.
4. Regenerate or update `wiki/` cards and `_index.md` files when source tables change.
5. Update `benchmarks/` protocol, input-set, deployment, method-source, scoring, or availability artifacts.
6. Update `reports/`, `index.md`, `README.md`, `RELEASE_NOTES.md`, and append `log.md`.
...
- `reports/wiki_validation_report.md` is generated by the validator.
