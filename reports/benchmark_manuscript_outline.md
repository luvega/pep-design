...
**面向近期 AI 多肽设计方法的 Benchmark 框架：任务分层、靶点集设计、可运行性审计与统一评分校准**

English working title: **A benchmark framework for recent AI peptide design methods: task stratification, target-set design, runnability auditing and calibrated scoring**
...
当前初稿中的 Results 部分应采用 planned results structure，而不是性能结论。第一部分报告文献和候选方法筛选结果；第二部分报告本地 Zotero benchmark lessons；第三部分报告 target/control schema；第四部分报告方法到 T1/T2/T3 的任务映射；第五部分报告 runnability audit 和 priority/readiness 分层；第六部分预留未来真实运行后的 parseability、runtime、output completeness、ranking calibration、interface metrics、developability proxies 和 failure cases。
...
当前限制包括：真实 target set 尚未冻结，部分候选方法权重和 license 待确认，部分方法需要 heavy dependency stack，D-peptide 和 cyclic peptide 的评分需要更明确的 stereochemistry-aware validation，affinity/ranking 指标仍需 assay-aware calibration。下一步应完成 Tier 1 smoke test、标准 target set、版本锁定、小规模结果报告和失败案例分析。
