...
本报告把补充材料 `蛋白-短肽对接，为什么不能只看分数.md` 转化为 Benchmark 评分协议的设计理由。它用于补强 `reports/benchmark_test_design_v1.md` 和 `benchmarks/protocols/scoring_outputs_schema.md` 的解释层，不报告任何真实 docking、MD、free energy 或实验结果。
...
本报告对应 `tables/scoring_metric_rationale_matrix_v1.1.csv` 中的 short-peptide rows，并在中英文大纲的 Unified Scoring Framework 小节中作为 v1.1 补充边界引用。
