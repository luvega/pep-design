...
| role | file | status |
|:---|:---|:---|
| 当前执行计划 | `reports/updated_plan_v0.9.md` | authoritative current plan |
| ARS 综合评审历史基线 | `reports/academic_research_suite_review_v0.6.md` | historical review gate |
| 服务器执行门槛 | `benchmarks/deployment/server_smoke_test_contract_v0.6.md` | active gate schema |
| 方法级 dry-run 合同 | `benchmarks/deployment/method_contracts/` | active planning contracts |
| license/schema/input-contract 审计 | `reports/license_schema_input_contract_review_v0.8.md` | active readiness evidence |
| Benchmark manuscript 主大纲 | `reports/benchmark_manuscript_outline.md` | active writing plan |
| Benchmark template 审计 | `reports/benchmark_template_audit.md` | active paper-structure gate |
| Introduction 六段链 | `reports/benchmark_intro_logic_chain.md` | active introduction skeleton |
| 综述驱动方法地形补充 | `reports/review_synthesis_benchmark_framework_supplement.md` | active v0.9 supplement |

旧文件 `reports/updated_plan_v0.6.md` 保留为历史版本，不再作为当前计划入口。
...
`benchmarks/method_sources/method_landscape_watchlist_v0.9.csv` 引入 27 个方法条目，用于显示方法范式、拓扑和覆盖缺口：
...
通过后，`reports/wiki_validation_report.md` 是当前机器可验证状态的权威记录。
