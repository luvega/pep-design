...
|:---|:---|:---|:---|
| Research Gap | Y | Existing peptide-design evaluation can conflate heterogeneous tasks, incompatible inputs/outputs, engineering readiness, ranking ability, developability proxies and biological evidence. Evidence: `reports/benchmark_manuscript_outline.md`; `benchmarks/protocols/benchmark_protocol_v0.md`. | Convert the gap into a concise comparison table against existing peptide-design benchmarks and scoring studies; keep limitations to at most three. |
| Construction Pipeline | Planned | The project has a construction route for KB, candidate methods, target/control schema, dataset watchlist, run.csv, server contracts, v0.8 license/schema/input-contract review and v0.9 method landscape mapping. Evidence: `scripts/build_benchmark_kb.py`; `tables/master_literature_manifest.csv`; `benchmarks/input_sets/target_set_v0_schema.md`; `benchmarks/deployment/server_smoke_test_contract_v0.6.md`; `reports/updated_plan_v0.9.md`. | In the manuscript, describe this as protocol construction and readiness-gate construction, not as dataset construction with final benchmark samples. |
| Evaluation Framework | Y | T1/T2/T3 task split, generation versus ranking/rescoring tracks, failure states, scoring-output schemas, negative/off-target panel and leakage labels are defined. Evidence: `benchmarks/protocols/benchmark_protocol_v0.md`; `benchmarks/scoring/scoring_protocol_v0.md`; `benchmarks/protocols/scoring_outputs_schema.md`. | Add a compact Figure/Table that maps each metric family to applicable output types and `not_applicable_reason`. |
