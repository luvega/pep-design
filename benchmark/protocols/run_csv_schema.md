...
| `method` | yes | 方法名，必须匹配 `kb/tables/candidate_method_scorecard.csv` |
...
## Optional v0.11 Adapter Columns

后续真实运行时可追加以下列，用于连接 `job_manifest.csv` 和 method adapter 输出。它们不是 v0 必填字段，不应破坏现有 `example_run.csv`：

| column | purpose |
|:---|:---|
| `parent_job_id` | 上游 `job_manifest.csv` 的 `job_id` |
| `source_output_id` | 原始方法输出文件名、record ID 或内部候选 ID |
| `generation_rank` | 方法原始候选 rank；无 rank 时留空 |
| `random_seed` | 与上游 job 或方法运行一致的 seed |
| `adapter_status` | `parsed`, `partial`, `failed`, `not_applicable`, or `not_real_benchmark` |
| `status_reason` | adapter 或 parser 状态说明 |
