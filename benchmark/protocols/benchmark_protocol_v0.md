...
Future benchmark targets are defined by `benchmark/input_sets/target_set_v0.csv`. Each target row must record target class, chain/structure provenance, positive control, negative or decoy controls, experimental affinity or assay evidence status, sequence/structure homology cluster and train-leakage risk.
...
真实执行前先通过 `job_manifest.csv` 描述 method-target-configuration job，再由方法适配器生成 method-specific input。方法原始输出先解析为 `candidate_outputs.csv`，再进入 `run.csv`、metric CSV 和 `merged_run.csv`。

该 adapter layer 只支持可审计运行接口，不支持性能结论。`method_output_manifest.csv` 必须记录 command、source commit、model revision、environment、external raw output root、runtime、exit code、parser status 和 failure state；缺少这些字段时只能写 planning/readiness，不得写 `smoke_test_ready`。
