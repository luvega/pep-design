# 多肽设计基准测试文章初稿大纲（中文版 v1.0，评审意见响应版）
...
本稿定位为协议先行的基准测试（Benchmark）论文初稿大纲，目标是界定近期 AI 多肽设计方法的公平、可复现、可审计比较方式。本稿不声称已经完成完整基准测试，不报告方法性能排名，也不声明任何候选方法已经在本地安装、运行或复现。

证据基础包括本项目知识库的 2021-06-03 至 2026-06-03 文献范围、10 个第一轮纳入方法、2 个观察名单方法、v0.9 方法地形图、候选数据集就绪状态表、服务器端空跑合同、可运行性审计、主张-证据映射表，以及本地 Zotero 中与基准测试和评分相关的文献总结。
...
生成式模型、蛋白基础模型和结构设计方法已进入多肽设计任务，当前文献和候选方法涉及线性多肽结合剂、环肽、D-肽、异手性多肽、微型蛋白结合剂以及蛋白-多肽相互作用等场景。不同方法在输入、输出、依赖、代码、权重、手性约束和评分适用性方面差异较大；不分层的统一排行榜会混淆生成能力、排序能力、工程可运行性和生物学证据。

本稿基于本地 Zotero/PD-wiki 知识库和外部元数据审计，提出一个面向近期 AI 多肽设计方法的协议先行基准测试框架。框架定义三类任务接口、10 个第一轮纳入方法、2 个观察名单方法、候选数据集来源、靶点/对照集模式定义、生成评测与排序/重打分评测双轨协议、方法可运行性与服务器端空跑门控，以及 `run.csv -> metric CSVs -> merged_run.csv` 的统一评分数据流；这些内容均为协议和 readiness 层证据，不是本地执行或性能结果。

AlphaFold 式排序、蛋白-多肽亲和力预测和多肽可开发性文献可用于设计评分与校准策略，但不能在未运行真实基准测试前支持方法优劣结论。本版本提供可复用的基准测试文章结构、证据边界、测试设计和后续执行待办事项，不给出最终性能排名或实验成功率。
...
图 1 使用同一多肽结合靶点展示三类路线的输入输出差异：仅序列方法接收靶点序列并输出多肽序列；结构条件化方法需要靶点 PDB、口袋或参考结合肽并输出多肽结构或复合物；微型蛋白基线方法输出结合体骨架和序列。该示例只说明任务接口差异，不展示方法优劣。

### 1.2 现有比较框架的缺口

当前比较框架存在三类缺口。任务错配会把仅序列多肽、结构条件化多肽和微型蛋白结合剂放入同一未分层榜单。就绪状态错配会把代码 URL、源码固定、服务器端合同或下载路线误读为已安装、已复现或已可批处理。证据层级错配会把结构置信度、亲和力预测、可开发性代理指标、负设计/脱靶特异性和生物学验证折叠为单一总分。

现有 peptide benchmark 与 scoring resources 已覆盖多肽性质预测、TCR/pMHC 序列转换、GPCR 靶点设计、结构语料和 binder success 等不同问题，但它们通常不同时处理近期生成方法的任务接口、源码/权重/许可证门控、输出可解析性和 claim-boundary 分层。正式稿应以表 0 或表 1 形式补充比较表；下表仅为引用计划层，不表示这些来源已经进入 `references.bib`，也不表示其中任何数据已下载或晋升为 frozen target set。

| 资源或数据源 | 主要用途 | 对本文有用的对话点 | 当前引用/证据边界 |
| --- | --- | --- | --- |
| PepBenchmark / PepBenchData | 序列与性质评测、可开发性候选 | 支持 developability/property 子任务对话，但不能直接替代 binder 生成 benchmark | `external:pepbenchmark_2026`; `needs_bibtex_verification` |
| Peptide Property Benchmark (PPB) | 多肽性质预测基准 | 支持性质预测与可开发性指标分离，但不证明候选设计方法性能 | `external:peptide_property_benchmark`; `needs_bibtex_verification` |
| TCRTransBench | pMHC/TCR-like 序列转换与交叉反应模式 | 支持 T1/pMHC 任务边界和 HLA/抗原肽元数据需求 | `external:tcrtransbench_2026`; `needs_bibtex_verification` |
| GPCR peptide design benchmark | GPCR 靶点类别与生成方法评测设计 | 支持靶点类别特异 benchmark 对话，但不能晋升当前 target set | `junker_assessment_2026`; `needs_bibtex_verification` |
| Overath binder-success dataset | de novo binder 成功/失败与排序校准候选 | 支持 T3 ranking/rescoring 和成功率标签的未来校准 | `external:overath_binder_success_2025`; `needs_bibtex_verification`; 未下载 |
| PepMerge / PepBDB / Q-BioLiP | 蛋白-多肽结构语料与泄漏参照 | 支持结构语料和 leakage reference 设计，但需另行核验许可与 schema | `arxiv:2411.10618`; `needs_bibtex_verification` |
...
本稿贡献限定在协议和 readiness 层：1）提出任务感知的基准测试协议，区分 T1/T2/T3 三类多肽设计任务；2）整理第一轮方法分类、外部代码路线和就绪门控，避免把 source pin 写成运行证据；3）设计候选数据集来源表、靶点/对照集模式定义和不下载数据的审计边界；4）提出生成评测与排序/重打分评测双轨测试设计，并建立中英文分稿、同步表、待办清单和主张门控。本文不提出新的多肽生成模型，也不报告 companion method、性能排名或实验验证。
...
表 1 基于 `kb/tables/candidate_method_classification_v1.csv` 展示候选方法分类。方法分为三层：
...
表 2 基于 `benchmark/input_sets/reference_dataset_sources_v1.csv` 总结参考数据集来源。候选来源包括 Overath binder-success dataset、PEPBI、PepMerge/PepBDB/Q-BioLip、PepMirror resources、Chang AF2 ranking cases、PepBenchmark/PepBenchData、GPCR peptide design benchmark 和 TCRTransBench。

这些来源只支持候选靶点发现、排序校准、模式定义设计或相关工作梳理。`target_set_v0.csv` 仍为仅含模式定义的文件；候选数据集或靶点候选不等于已冻结的基准测试靶点。所有数据源保持未下载或待核验状态。
...
生成评测记录方法能否从标准化输入产生可解析、有效且任务兼容的输出。核心字段包括输出完整性、序列/PDB 可解析性、长度有效性、链标识有效性、手性标记、环化标记、非天然残基标记、失败状态、运行时间和资源元数据。

主索引为 `run.csv`。结构任务默认结合体链为 `A`，靶点链为 `B`；多链靶点用 `B,C,D...` 记录。任何方法特异的链命名约定都必须通过适配层保留从原始链到标准链的映射。

## 8. 排序与重打分评测协议

排序与重打分评测记录亲和力、结构、界面和可开发性证据能否合理排列已有候选或生成候选。该轨道与生成评测分开报告，计划记录已知结合剂排序、负对照分离度、top-k 富集、校准误差和不适用原因。当前阶段只定义字段，不填入性能结果。
...
就绪门控包括 `metadata_ready`、`source_pinned`、`license_checked`、`weights_manifested`、`input_contract_ready`、`dry_run_ready` 和 `smoke_test_ready`。PepMLM 与 RFdiffusion + ProteinMPNN 已有 v0.7 服务器端合同和 v0.8 输入合同就绪线索；PepMirror 因 PyRosetta、Vina、OpenMM 和检查点路线仍保持依赖合同状态，不进入空跑就绪状态。

这些状态仅用于安排后续服务器端预检查，不代表本地复现。后续服务器执行必须使用外部代码克隆、数据和权重根目录；本知识库不接收第三方源码、数据、权重或 GPU 结果。
...
这些条目应写成 readiness findings 或 planned result slots，而不是 Benchmark 结果型 *Finding X*。不能写入任何方法性能优劣、命中率、实验成功率或本地复现结论。

## 13. 讨论

本稿的核心观点是，多肽设计基准测试应先解决任务定义、靶点/对照集、泄漏控制、工程可运行性和评分适用性，再讨论性能排序。计算评分只能支持候选排序和结构假设，不能替代实验亲和力、实验结构、细胞功能、PK/PD 或 CMC 证据。结合评分也不能替代药物化学可开发性证据。

当前缺失的关键测量包括：服务器端真实执行日志、commit 与环境记录、真实 `run.csv`、metric CSVs、`merged_run.csv`、输出 parser 结果、失败率、运行时间、资源消耗、target/control freeze、wet-lab affinity/function、off-target specificity、可开发性实验、D-肽/环肽/ncAA 的手性与拓扑验证，以及训练集泄漏/同源性核验。纳入这些测量后，框架中的任务分层和评分字段可能得到加强；如果真实运行显示某些方法无法批处理、输出不可解析或评分不适用，则会削弱任何关于可比性的强表述。

本框架的外推范围也应谨慎限定。任务分层、claim gate、not-applicable metric 和 source/license/readiness gate 可迁移到其他 peptide-design benchmark；但性能、runnability、biological success、target novelty 和 developability 结论不能外推到新的 target class、assay、chemistry、chirality/cyclization representation 或 server environment。GPCR、TCR/pMHC、PPI 界面、环肽、D-肽和非天然氨基酸多肽都可能改变可评估输出、对照集和评分适用性。

局限包括：`target_set_v0.csv` 仍为 0 个 frozen rows，部分数据集的许可证、模式定义、对照和泄漏风险仍需核验，部分方法权重和依赖尚未解决，D-肽、环肽和含非天然氨基酸多肽的输出需要手性敏感验证，真实冒烟测试和性能报告尚未开始。下一步应进入服务器端预检查包，而不是直接撰写性能结论。
...
本节将原“图表计划”改为图表草稿。四张图已使用内置 `$imagegen` 重绘为多层级概念图，用于解释论文结构、测试协议和证据边界，不代表真实 benchmark 结果。四张嵌入表均从项目 CSV 提取；新增的现有 benchmark / scoring resources 比较表保留在引言中，属于引用计划层，不代表外部来源已完成 BibTeX 核验或数据下载。完整字段和未截断内容以原 CSV 为准。

### 图 1. 基准测试贯穿示例与总体流程

![图 1：基准测试贯穿示例与总体流程](../assets/figures/benchmark_figure1_overview_v1.png)

图 1 以同一 peptide-binding 靶点场景为贯穿示例，上层区分 T1/T2/T3 三类输入输出，中层展示 source layer、method taxonomy、target/control schema、execution gates 和 scoring interface，下层用 claim gate 区分当前可写的 readiness findings 与未来需要服务器和实验支持的证据。该图只说明协议结构，不表示任何方法已经完成本地复现。

### 图 2. 任务-方法-靶点矩阵

![图 2：任务-方法-靶点矩阵](../assets/figures/benchmark_figure2_task_method_matrix_v1.png)

图 2 将 T1、T2、T3 三类任务按 input/output、first-wave methods、target/control evidence 和 scoring applicability 展开。该矩阵强调后续比较应优先在同一任务内进行；跨任务只比较接口、可运行性和证据缺口，不混排为单一榜单。

### 图 3. 生成评测与排序/重打分评测双轨结构

![图 3：生成评测与排序/重打分评测双轨结构](../assets/figures/benchmark_figure3_dual_track_v1.png)

图 3 将生成评测与排序/重打分评测拆成两条证据轨道，并在底部列出仍缺失的 execution logs、真实 `run.csv`/metric CSV、parser outputs、wet-lab affinity/function、off-target specificity、developability experiments 和 chirality/cyclization/ncAA validation。生成评测关注输出是否可解析、是否满足任务约束和失败率；排序/重打分评测关注候选集合、对照、代理指标、校准和富集趋势。

### 图 4. 评分架构与 CSV 合并流程

![图 4：评分架构与 CSV 合并流程](../assets/figures/benchmark_figure4_scoring_architecture_v1.png)

图 4 对应 `run.csv -> metric CSVs -> merged_run.csv` 的数据流。结构置信度、界面几何、结构相似性、Rosetta/能量项、可开发性、负设计和 leakage/homology 相关指标应由独立 CSV 输出，再按 `design_id` 或 `target_id` 合并；不可用指标必须保留 `status` 与 `not_applicable_reason`，空字段不能被解释为零分。

### 表 1. 候选方法分类、任务、输入输出和当前门控

来源：`kb/tables/candidate_method_classification_v1.csv`。
...
来源：`benchmark/input_sets/reference_dataset_sources_v1.csv`。
...
来源：`kb/tables/scoring_metric_rationale_matrix_v1.1.csv`。
...
来源：`benchmark/deployment/method_readiness_review_v0.8.csv`。
...
待办事项详见 `manuscript/support/benchmark_manuscript_todo_v1.csv`。优先级最高的任务为：补充图 1 的贯穿示例，完成人工复核表 1 和表 2，使用 citation-management 校验 citation key，并在服务器端执行前关闭 PepMLM、RFdiffusion + ProteinMPNN 和 PepMirror 的许可证、检查点和输入合同开放项。

## 17. 参考文献与引用边界

引用清单详见 `manuscript/support/benchmark_reference_bibliography_v1.md`。正文引用优先使用现有 BibTeX key；外部来源如 Overath、PepBenchmark、TCRTransBench 和 GPCR benchmark 在未进入 `references.bib` 前必须标注 `needs_bibtex_verification`。不得新增无法追溯的引用。
