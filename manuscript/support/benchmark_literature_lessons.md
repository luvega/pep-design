...
本报告只总结本地 Zotero 文献库中与 Benchmark、评分、affinity prediction 和 peptide developability 相关的启示。Zotero 保持只读；Zotero item key 和 BibTeX key 分开记录在 `kb/tables/benchmark_literature_lessons.csv`。这些文献不被写成“最终评分工具已确定”，而用于约束本项目的评分、校准和文章主张边界。
...
See `kb/tables/benchmark_literature_lessons.csv`.
